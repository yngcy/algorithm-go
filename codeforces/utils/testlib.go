...
func AssertEqualStringCaseWithPrefix(t *testing.T, testCases [][2]string, targetCaseNum int, runFunc ioFunc, prefix string) {
	...
	for curCaseNum, tc := range testCases {
		...
		t.Run(fmt.Sprintf("%sCase %d", prefix, curCaseNum+1), func(t *testing.T) {
			...
			// 还有未读入的内容
			if mockReader.Len() > 0 {
				t.Log("Warning: there are still some bytes left in the input")
			}
...
// 无尽对拍测试
// inputGenerator 生成随机数据，runFuncAC 为暴力逻辑或已 AC 逻辑，runFunc 为当前测试的逻辑
func AssertEqualResultsInf(t *testing.T, inputGenerator func() string, runFuncAC, runFunc ioFunc) {
	for tc := 1; ; tc++ {
		...
		if DisableLogInput {
			...
			assert.Equal(t, expectedOutput, actualOutput, "Wrong Anser %d\nInput:\n%s", tc, input)
		}

		// 每到 2 的幂次就答应检测了多少个测试数据
		if tc&(tc-1) == 0 {
...
// 无尽验证测试
// inputGenerator 除了返回随机输入数据外，还需要返回一个闭包，这个闭包接受 runFunc 的输出结果，根据输出数据验证输出结果是否正确
